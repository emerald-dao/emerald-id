...
access(all) contract EmeraldIdentityShadow {
    ...
    access(all) view fun getDiscordFromAccount(account: Address): String?  {
        let admin = EmeraldIdentityShadow.account.storage.borrow<&Administrator>(from: EmeraldIdentityShadow.AdministratorStoragePath)!
    ...
    init() {
        ...
        self.account.storage.save(<- create Administrator(), to: EmeraldIdentityShadow.AdministratorStoragePath)
